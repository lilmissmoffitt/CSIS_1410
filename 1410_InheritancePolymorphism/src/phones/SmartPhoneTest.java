...
/** 
 * * Tests the constructor and the public methods from * class SmartPhone. * * =
...
class SmartPhoneTest { 
	...
	void smartPhone_initializesFields() {
		SmartPhone smartPhone1 = new SmartPhone("Samsung Galaxy", new Dimension(150.9, 72.6, 7.7), 64);
	...
	void smartPhone_negativeStorage_IllegalArgumentException() {
		Exception exception = assertThrows(IllegalArgumentException.class,
				() -> new SmartPhone("Samsung Galaxy", new Dimension(150.9, 72.6, 7.7), -64));
		assertTrue(exception.getMessage().matches(".*[S|s]torage needs to be assigned a positive value.?"));
	}

	@Test
	void smartPhone_zeroStorage_IllegalArgumentException() {
		assertThrows(IllegalArgumentException.class,
				() -> new SmartPhone("Samsung Galaxy", new Dimension(150.9, 72.6, 7.7), 0));
	}

	@Test
	void call_returnsMessageIncludingTheNumberProvided() {
		assertEquals("Calling 8015817200 by selecting the number", smartPhone.call(8015817200L));
	}
	...
	void takePicture_randomlyReturnsOneOfTwoMessages() {
		...
		for (int i = 0; i < 1000; i++) {
			...
			if (message.matches("[T|t]aking a horizontal picture.?"))
				...
				verticalCount++;
			else fail("The message " + message + " was none of the allowed choices.");
		}
